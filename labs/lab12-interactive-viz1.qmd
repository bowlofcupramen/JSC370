...
```{python}
# Note that I had to change the directories here since it would not
# see the files somehow when I only use the file name or use ./
sb_locs = pd.read_csv("../labs/starbucks-locations.csv")

sb_nutr = pd.read_csv("../labs/starbucks-menu-nutrition.csv")
sb_nutr.columns = sb_nutr.columns.str.strip()

usa_pop = pd.read_csv("../labs/us_state_pop.csv")

usa_states = pd.read_csv("../labs/states.csv")
```
...
fig = px.scatter(
    ...
    labels={'population': 'Population', 'n_tores': 'Number of Stores'}
)
...
- 4a) Answer:
The number of Starbucks does appear to be proportional to the population of the state. 
...
- 4b) Answer:
It is indeed different. The mean calorie of drinks is much lower than that of foods, and appears to have less variance as well.
...
- 4c) Answer:
The top 20 can be seen in the plot, so I will only list the first 3, which are: sanwich, egg, and cake.
...
- 4d) Answer:
There does appear to be a relationship between the carb mass and calories, being that more carbs tend to mean more calories. 
Foods tend to have more calories than drinks.
...
- 5) Answer:
Sandwich is the top word that is associated with the most calories.
...
- 6) Answer:
They are all roughly positively correlated with each other.
...
- 7) Answer:
The two graphs are extremely similar, but the relation does not appear to be perfectly linear as many states with proportionaly higher population have less stores than expected (i.e. lighter color).
...
def update(selected_var):
    ...
    bar_fig = px.bar(
        bar_df, x='top word', y=selected_var,  # YOUR CODE HERE
    ...
    scatter_fig = px.scatter(
        sb_top, x='top word', y=selected_var,  # YOUR CODE HERE
        color="Category",
        hover_data=["Item", "top_word"],
