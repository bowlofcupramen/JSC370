...
It reports 24 cores, but only one was allocated. This matches the 1 core requested since there was only one job. 
...
Job 202851 on squid01 | CPUs allocated: 1
...
Job 202853 on squid01 | CPUs allocated: 4
...
|--------|----------------|----------------|------------------|---------|
| `flu_1core.sh` | 1 | 2.89 | 2.55 | 1.13 |
| `flu_4core.sh` | 4 | 2.79 | 0.93 | 3.00 |

*Is the speedup on the cluster similar to what you observed on your laptop in Lab 10? What factors might cause differences?*
It's slower than on my laptop on lab 10. It might be a difference in the CPU speed and/or GPU speed.
...
task=0  vacc=0 (0%)  workers=4  time=0.89s
...
task=2  vacc=450 (50%)  workers=4  time=1.17s
...
task=4  vacc=900 (100%)  workers=4  time=1.39s
...
The mean deceased change little from 0% vaccination to 50%. At 75%, the mean deceased decrease greatly, reducing by over half. It drops a lot again at 100%, but not as much as from 50% to 75%.
---
...
for v in vacc_levels:
    arr = np.load(f"../labs/results_vacc{v}.npy")
...
That there is little change in mean deceased until the majority (>50%) are immune. 
...
2
24
nproc matches the cpis per task-2.
...
*Paste the output.*
Done in 0.25s, sum=333332833333500000
...
*Your answers here.*
1. SLURM's role is to manage the computational resources. 
We are not allowed to SSH directly to avoid abuse of the resources (such as hoarding the resources with a long running program).

2. A SLURM job array is a group of jobs in one shell script. It is more convenient than doing individual batch scripts when the content of the jobs are similar in nature, such as hyperparameter tuning for models.

3. The speedup on local was around 4-5 times, while it was 3 times on the cluster. While there are some differences, the differences was not too large. Hardware differences could be that my local device has faster CPUs and/or GPUs. 

4. It would have taken 5.71s. Assuming the jobs started at the same time, it took 1.39s total with the job array. 
