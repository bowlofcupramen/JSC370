...
def get_df_year(url, df, pg, year):
  ...
  for i in range(1, pg + 1):
    ...
    if count >= prev:
      prev = count
      # count is just to help me track progress
      # print(count)
    time.sleep(0.1)
...
X = merged[["year", "gdp", "consum"]].values
y = merged["lf_exp"].values
X_const = sm.add_constant(X)
mlr = sm.OLS(y, X_const).fit()
print(mlr.summary())
```
```{python}
gam_mod = LinearGAM(
...
Provide the following in your analyses:
  ...
  For the linear model, the constant term is around -302, while the coefficients of the year, GDP, and alcohol consumption terms are 0.1823, 0.0003, and 0.4912 respectively. Based on the test statsitics, non of these values are statistically significant. Based on the R-squared, both adjusted and otherwise (0.484 and 0.481, respectively), the model does not perform too well. 
...
spec = [
    (1, 1, "GDP per capita ($USD)",  "blue", "Effect of GDP"),
    (2, 2, "Alcohol Consumption per capita (L)", "red",  "Effect of Alcohol Consumption")
]
...
plt.show()
```
  For the GAM, the pseudo R-squared is about 0.6942, which is much better than the linear regression model. The EDoF for year and GDP per capita are also both statistically significant, while the EDoF for the GDP per capita and alcohol consumption are both relatively high (>10). From the plot, the GDP is indeed quite non-linear, though for the effects of alcohol consumption, even though there are some noticeable non-linearity, the overall plot is very horizontal, and it is possible that the feature could be replaced by a linear one. In terms of effects, GDP does seem to be correlated with higher life expectancy, and this correlated increase is especially steep for lower GDP per capita countries.
